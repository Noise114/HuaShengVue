<template>
  ...
  <script>
  import { listInfo, getInfo, delInfo, addInfo, updateInfo } from "@/api/huasheng/gameinfo";
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('是否确认删除【游戏基础数据】编号为"' + gameIds + '"的数据项？').then(function() {
          return delInfo(gameIds);
        }).then(() => {
          this.getList();
